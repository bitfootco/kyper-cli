...
var initCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// Generate .kyperignore if it doesn't already exist
		if _, err := os.Stat(".kyperignore"); os.IsNotExist(err) {
			kyperignoreContent := buildKyperignore(stackNames)
			if err := os.WriteFile(".kyperignore", []byte(kyperignoreContent), 0644); err != nil {
				ui.PrintWarning(fmt.Sprintf("Could not create .kyperignore: %v", err))
			} else {
				ui.PrintSuccess("Created .kyperignore with sensible defaults")
			}
		}
...
func buildKyperignore(stacks []string) string {
	var b strings.Builder
	b.WriteString("# .kyperignore — patterns to exclude from the Kyper upload archive.\n")
	b.WriteString("# Syntax: one pattern per line, # for comments, trailing / for directories.\n")
	b.WriteString("# This file was generated by `kyper init`. Edit freely.\n")
	b.WriteString("\n")
	b.WriteString("# Environment & secrets (never upload these)\n")
	b.WriteString(".env\n")
	b.WriteString(".env.*\n")
